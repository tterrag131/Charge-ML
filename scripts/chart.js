...
const MultiMetricsCalculator = {
    ...
    calculateMultiMetrics(data) {
        if (!data?.Ledger_Information?.metrics) {
        ...
        // Use ShipComp (Shipping Completion) as our multi-rate
        const currentMultiRate = metrics.ShipComp ? 
            metrics.ShipComp[metrics.ShipComp.length - 1] : 0;

        // Use DOBL (Daily Order Backlog) for target calculation
        const avgDOBL = metrics.DOBL ? 
            metrics.DOBL.reduce((sum, val) => sum + val, 0) / metrics.DOBL.length : 0;

        // Calculate target rate based on time of day
        const currentHour = new Date().getHours();
        const targetRate = (currentHour / 24) * 100; // Simple linear target

        return {
            avgIPTM: avgDOBL * 100, // Convert to percentage
            currentMultiRate: currentMultiRate,
            targetMultiRate: targetRate,
            deviation: this.getMultiTargetDeviation(currentMultiRate, targetRate),
            isOnTarget: Math.abs(this.getMultiTargetDeviation(currentMultiRate, targetRate)) <= 5
        };
    },
    ...
    calculateMultiSplit(value, percentage) {
...
const UpdateFunctions = {
    updateDashboard(data) {
        ...
        this.currentMultiMetrics = MultiMetricsCalculator.calculateMultiMetrics(data);

        // Update metrics based on current view
        if (this.isShowingTomorrow) {
    ...
    updateMetricWithMultiSplit(totalElement, multiElement, value) {
        if (!totalElement || !multiElement) {
            console.warn('Missing elements for metric update');
            return;
        }

        try {
            // Get the multi percentage from current metrics
            const multiPercentage = this.currentMultiMetrics?.avgIPTM || 0;
            
            // Calculate values
            const totalValue = Math.round(value || 0);
            const multiValue = MultiMetricsCalculator.calculateMultiSplit(totalValue, multiPercentage);

            // Update display
            totalElement.textContent = totalValue.toLocaleString();
            multiElement.textContent = `(${multiValue.toLocaleString()})`;

            // Calculate and apply styling based on deviation
            const actualPercentage = totalValue > 0 ? (multiValue / totalValue) * 100 : 0;
            const deviation = Math.abs(actualPercentage - multiPercentage);
            ...
            // Debug log
            console.log('Multi-split calculation:', {
                total: totalValue,
                multi: multiValue,
            ...
            console.error('Error updating metric with multi split:', error);
        }
    },

    // Add the missing updateProcessingMetrics function
    ...
    updateMultiMetrics(data) {
        ...
        // Get the latest shipping completion rate
        const currentShipComp = metrics.ShipComp ? 
            metrics.ShipComp[metrics.ShipComp.length - 1] : 0;

        // Update display elements
        this.safeUpdateElement('currentIPTM', `${currentShipComp.toFixed(1)}%`);
        this.safeUpdateElement('multiIPT', metrics.IPTNW ? 
            metrics.IPTNW[0].toLocaleString() : '0');
        this.safeUpdateElement('currentMSSF', `${currentShipComp.toFixed(1)}%`);

        // Update progress bars
        this.updateProgressBars(dayProgress, currentShipComp);
        
        // Update progress status
        this.updateProgressStatus(currentShipComp, dayProgress);

        if (this.currentMultiMetrics) {
            this.updateMultiProgress(this.currentMultiMetrics);
        }
    },

    // Helper functions to safely update elements
    ...
    updateProgressBars(dayProgress, currentMSSF) {
        ...
        if (dayProgressFill) {
            ...
            this.safeUpdateElement('dayProgressValue', `${dayProgress.toFixed(1)}%`);
        }
        
        if (multiProgressFill) {
            multiProgressFill.style.width = `${currentMSSF}%`;
            this.safeUpdateElement('multiProgressValue', `${currentMSSF.toFixed(1)}%`);
        }
    },

    updateProgressStatus(currentMSSF, dayProgress) {
    ...
    updateMultiProgress(multiMetrics) {
        if (!multiMetrics) return;

        try {
            // Update percentages
            this.safeUpdateElement('targetMultiPercentage', 
                `${multiMetrics.avgIPTM.toFixed(1)}%`);
            this.safeUpdateElement('currentMultiPercentage', 
                `${multiMetrics.currentMultiRate.toFixed(1)}%`);

            // Update progress bar
            const progressFill = document.querySelector('#multiProgressBar .progress-fill');
            if (progressFill) {
                const percentage = (multiMetrics.currentMultiRate / multiMetrics.targetMultiRate) * 100;
                progressFill.style.width = `${Math.min(percentage, 100)}%`;
                progressFill.style.backgroundColor = 
                    percentage < 90 ? '#e74c3c' : 
                    percentage < 95 ? '#f1c40f' : 
                    '#2ecc71';
            }

            // Debug log
            console.log('Multi metrics update:', {
                target: multiMetrics.avgIPTM,
                current: multiMetrics.currentMultiRate,
                isOnTarget: multiMetrics.isOnTarget
            });

        } catch (error) {
            console.error('Error updating multi progress:', error);
        }
    },
