...
class ChargePredictor:
    ...
    def train_sarima_model(self):
        ...
        try:
            ...
            self.ts_data = self.df.set_index('Time')['Workable'].resample('h').mean()
            self.ts_data = self.ts_data.fillna(method='ffill')
    ...
    def _get_same_day_data(self, target_date):
        """Get data from earlier hours of the same day"""
        try:
            current_hour = target_date.hour
            
            # Get data from the same day up until the current hour
            same_day_data = self.df[
                (self.df['Time'].dt.date == target_date.date()) &
                (self.df['Time'].dt.hour < current_hour)
            ].copy()
            
            if not same_day_data.empty:
                print(f"\nFound {len(same_day_data)} records for current day")
                print(f"Hours available: 0 to {current_hour-1}")
            
            return same_day_data
            
        except Exception as e:
            print(f"Error in _get_same_day_data: {str(e)}")
            return pd.DataFrame()
    
    def _calculate_day_trend(self, same_day_data):
        """Calculate the trend from same-day data"""
        if same_day_data.empty:
            return 0
        
        # Calculate the slope of the trend
        hours = same_day_data['Time'].dt.hour.values
        values = same_day_data['Workable'].values
        if len(hours) > 1:
    ...
    def predict_for_date(self, target_date):
        """Make predictions for a specific date with same-day data enhancement"""
        try:
            target_date = pd.to_datetime(target_date)
            
            # Get same-day data
            same_day_data = self._get_same_day_data(target_date)
            day_trend = self._calculate_day_trend(same_day_data)
            
            # Create a date range for 24 hours of the target date
            prediction_dates = pd.date_range(
                start=target_date.replace(hour=0),
                end=target_date.replace(hour=23),
                freq='h'
            )
            ...
            # Enhance predictions with same-day data
            enhanced_predictions = base_predictions.copy()
            current_hour = target_date.hour
            
            if not same_day_data.empty:
                # Use actual values for past hours
                for hour in range(current_hour):
                    hour_data = same_day_data[same_day_data['Time'].dt.hour == hour]
                    if not hour_data.empty:
                        enhanced_predictions[hour] = hour_data['Workable'].iloc[0]
                
                # Adjust future predictions based on the day's trend
                trend_weight = 0.3  # Adjustable weight for trend influence
                for hour in range(current_hour, 24):
                    hours_ahead = hour - current_hour
                    trend_adjustment = day_trend * hours_ahead * trend_weight
                    enhanced_predictions[hour] += trend_adjustment
            
            # Ensure predictions are non-negative
            enhanced_predictions = np.maximum(enhanced_predictions, 0)
            
            return enhanced_predictions
    
        except Exception as e:
            print(f"Error in predict_for_date: {str(e)}")
            return None
    ...
    def plot_prediction(self, target_date_str):
        ...
        try:
            ...
            predictions = self.predict_for_date(target_date)
            hours = range(24)
            
            # Get historical data from previous year
            prev_year = target_date - pd.DateOffset(years=1)
            historical_data = self.df[
                (self.df['Time'].dt.month == prev_year.month) & 
                (self.df['Time'].dt.day == prev_year.day) &
                (self.df['Time'].dt.year == prev_year.year)
            ]
            ...
            # Plot predictions and historical data
            ax1.plot(hours, predictions, 'b-', label='SARIMA Prediction', linewidth=2)
            
            if not historical_data.empty:
                historical_values = historical_data.sort_values('Hour')['Workable'].values
            ...
            results_df = pd.DataFrame({
                'Hour': hours,
                'SARIMA_Prediction': predictions
            })
            
            # Add historical data if available
            if not historical_data.empty:
                results_df['Previous_Year'] = historical_values
            if not historical_data_2yrs.empty:
    ...
    def evaluate_model(self):
        """Evaluate the enhanced SARIMA model performance"""
        try:
            # Calculate in-sample predictions
            in_sample_predictions = self.sarima_results.get_prediction(start=self.ts_data.index[0])
            in_sample_mean = in_sample_predictions.predicted_mean
            
            # Calculate metrics
            mse = np.mean((self.ts_data - in_sample_mean) ** 2)
            rmse = np.sqrt(mse)
            mae = np.mean(np.abs(self.ts_data - in_sample_mean))
...
class ChargePredictorGUI:
    ...
    def generate_prediction(self):
        ...
        try:
            ...
            predictions = self.predictor.predict_for_date(target_date)
            hours = range(24)
            ...
            # Get historical data
            prev_year = target_date - pd.DateOffset(years=1)
            historical_data = self.predictor.df[
                (self.predictor.df['Time'].dt.month == prev_year.month) & 
                (self.predictor.df['Time'].dt.day == prev_year.day) &
                (self.predictor.df['Time'].dt.year == prev_year.year)
            ]
            ...
            # Plot the data
            results_df.plot(x='Hour', 
                           y=['SARIMA_Prediction', 'Previous_Year'] if 'Previous_Year' in results_df.columns 
                           else ['SARIMA_Prediction'],
                           ax=ax1, 
                           marker='o')
            
            ax1.set_title(f'Charge Prediction for {selected_date}')
            ax1.set_xlabel('Hour of Day')
            ax1.set_ylabel('Predicted Charge')
            ax1.grid(True, alpha=0.3)
            ax1.legend()
            ...
            self.update_results_tables(results_df)
            
        except Exception as e:
            print(f"Error in generate_prediction: {str(e)}")
            tk.messagebox.showerror("Error", f"Failed to generate prediction: {str(e)}")
    
    def update_results_tables(self, results_df):
        """Update the results tables in different tabs"""
        try:
            ...
            for widget in self.ml_frame.winfo_children():
                widget.destroy()
            for widget in self.previous_frame.winfo_children():
                widget.destroy()
                
            # Create treeview for combined predictions
            tree = ttk.Treeview(self.combined_frame)
            ...
            for i in range(len(results_df)):
                tree.insert('', 'end', values=results_df.iloc[i].tolist())
                
            # Add scrollbar
            scrollbar = ttk.Scrollbar(self.combined_frame, orient="vertical", command=tree.yview)
            ...
            print(f"Error in update_results_tables: {str(e)}")
